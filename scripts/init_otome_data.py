...
# Create a new engine for the script to avoid event loop issues with global engine
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession
from sqlalchemy.orm import sessionmaker
from app.core.config import settings

script_engine = create_async_engine(settings.SQLALCHEMY_DATABASE_URI, echo=True)
ScriptSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=script_engine, class_=AsyncSession)
...
async def reset_database():
    ...
    # Dispose of the script engine to close connections
...
async def init_data():
    ...
    # Re-create engine/session for data init to be safe
    # Use the same script_engine structure but new instance if needed, or just reuse if not disposed?
    # Actually, let's create a fresh one to be absolutely sure about event loop binding
    
    # Must use the loop that runs this
    init_engine = create_async_engine(settings.SQLALCHEMY_DATABASE_URI, echo=True)
    InitSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=init_engine, class_=AsyncSession)

    try:
        async with InitSessionLocal() as db:
            ...
            characters_data = [
                {
                    ...
                    "gender": "MALE",
                    "expression_style": "言简意赅，逻辑缜密。善于使用反问句，情绪波动极少，除非涉及他在意的人。在指出错误时一针见血，毫不留情。",
                ...
                },
                {
                    ...
                    "gender": "MALE",
                    "expression_style": "热情洋溢，充满活力。喜欢使用感叹号和可爱的语气词。说话直率坦诚，毫不掩饰自己的情感。",
                ...
                },
                {
                    ...
                    "gender": "MALE",
                    "expression_style": "隐晦迷离，充满艺术气息。喜欢使用比喻和象征，说话留有余地，引人遐想。语调经常带着一丝戏谑。",
